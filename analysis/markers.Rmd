...
```{r}
res <- findMarkers(sce, sce$cluster, test.type = "t", pval.type = "any", direction = "up", lfc = log2(1.5))
...
Create a heatmap of expression values for the top 50 marker genes:
...
for (n in names(sig)) {
  ...
  plotHeatmap(sce, colour_columns_by = "cluster", features = ids, center = TRUE, zlim = c(-1, 1), symmetric = TRUE)
...
Create dimensionality reduction plots coloured by the top 12 marker genes:
...
for (n in names(sig)) {
  ...
  print(patchwork::wrap_plots(plt, ncol = 4))
...
Plot expression values for the top 10 marker genes against cluster number:
...
```{r}
res <- findMarkers(sce, sce$cluster, test.type = "t", pval.type = "all", direction = "up", lfc = log2(1.5))
...
```{r}
res <- findMarkers(sce, sce$cluster, test.type = "wilcox", pval.type = "any", direction = "up", lfc = log2(1.5))
...
```{r}
res <- findMarkers(sce, sce$cluster, test.type = "wilcox", pval.type = "all", direction = "up", lfc = log2(1.5))
